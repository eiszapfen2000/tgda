...
With multiple patterns at hand, where patterns consist of up to nine separate
spectra each, pattern generation had become computationally too expensive to
still allow for real-time framerates. Therefore we took a number of measures
to restore real-time behaviour. First, we moved pattern spectrum generation
and transformation via \FFT to separate threads each (Section~\ref{sec:demo_application}).
Second, we made sure to compute only the time-dependent part of the spectrum
for each frame (Section~\ref{sec:spectrum_synthesis}).
Third, as all of our spectra are hermitian, we can compute the \InvFourierTransform
of two spectra at once (Section~\ref{sec:discrete_fourier_transform}). Hence,
we were able to reduce the maximum number of \FourierTransforms per pattern
from nine to five.
