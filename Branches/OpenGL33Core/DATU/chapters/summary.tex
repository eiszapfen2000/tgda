...
(Section~\ref{sec:level_of_detail}).
...
expensive to still allow for real-time framerates.
...
The reduced resolution lead to an improvement in performance, and moreover,
we found that it did not seem to degrade visual quality in a significant way
for a large range of distinct scenes.
%For a large range of distinct scenes the reduced resolution did not seem
%to degrade visual quality in a significant way.
Thus we decided to stick with the multi-resolution approach, as it allows
for a more fine-grained balance between model detail and performance.
%
%
