...
Gerstner's work \citep{Gerstner:1809, Rankine:1863} to synthesize waves. Gerstner
waves assume a water surface dominated by gravity and an incompressible water body of infinite depth. On such a water surface, Gerstner waves may take on more varied
shapes than sinusoids are able to model, e.g., waves with a sharp crest.}
A single Gerstner wave may be written as follows:
\begin{align}
...
\textcolor{changed}{Similarly, \cite{Centelles:2014} also outline an adaptive
meshing scheme which seeks to provide more detail close to the viewer.
Their approach starts with a coarse mesh in world space, which is then
dynamically refined dependent on the camera's current position and the camera's 
respective view frustum. The actual refinement is computed inside a geometry shader, adding or removing triangles as needed. As the mesh refinement
algorithm is an iterative one, \citeauthor{Centelles:2014} store the results of intermediate iterations to GPU memory. By reusing said results in subsequent frames, the number of required iterations to reach the desired mesh granularity
