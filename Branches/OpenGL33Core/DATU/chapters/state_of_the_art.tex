...
\textcolor{changed}{Instead of using sinusoids,
\citet{Fournier:1986} build on Gerstner's work \citep{Gerstner:1809, Rankine:1863}
to synthesize waves. Compared to the gentle form of sinusoids
in \citet{Max:1981}, Gerstner waves allow for a larger variety of
wave shapes, including waves with a sharp crest. Such waves are common
near the shore and in stormy seas.
...
\begin{figure}
 ...
 \subtop[]
 {
  \includegraphics[width=0.475\textwidth]{figures/Simulating_Ocean_Water-012.png}
	\label{fig:tessendorf_choppy_waves:1}
 }
 \hfill
 \subtop[]
 {
  \includegraphics[width=0.475\textwidth]{figures/Simulating_Ocean_Water-013.png}
...
\subtop[]
{
	\includegraphics[width=0.475\textwidth]{figures/Empirical_Directional_Wave_Spectra_for_Computer_Graphics_-_Horvath_2015-011.png}
	\label{fig:horvath_swell:1}
}
\hfill
\subtop[]
{
	\includegraphics[width=0.475\textwidth]{figures/Empirical_Directional_Wave_Spectra_for_Computer_Graphics_-_Horvath_2015-013.png}
	\label{fig:horvath_swell:2}
}
\caption[The same ocean with different amounts of swell \citep{Horvath:2015}.]{
\textcolor{changed}{
\subcaptionref{fig:horvath_swell:1} The ocean without swell.
\subcaptionref{fig:horvath_swell:2} The same ocean, but dominated by swell.
Source: \citet{Horvath:2015}
}
}
\label{fig:horvath_swell}
\end{figure}
%
%
\textcolor{changed}{
...
%The water waves may reflect, refract, and diffract
%around objects
}
...
%\subtop[]
%{
%	\includegraphics[scale=0.125]{figures/View-Dependent_Tessellation_and_Simulation_of_Ocean_Surfaces_-_Puig-Centelles_2014-001.png}
%}
%\hfill
%\subtop[]
%{
%	\includegraphics[scale=0.125]{figures/View-Dependent_Tessellation_and_Simulation_of_Ocean_Surfaces_-_Puig-Centelles_2014-002.png}
%}
%\hfill
%\subtop[]
%{
%	\includegraphics[scale=0.125]{figures/View-Dependent_Tessellation_and_Simulation_of_Ocean_Surfaces_-_Puig-Centelles_2014-003.png}
%}
%\hfill
%\subtop[]
%{
%	\includegraphics[scale=0.125]{figures/View-Dependent_Tessellation_and_Simulation_of_Ocean_Surfaces_-_Puig-Centelles_2014-004.png}
%}
...
%}
%Moreover, \citeauthor{Centelles:2014} make sure that in addition to
%the mesh refinement, the computation of the surface displacements is
%limited to the current view frustum, too.
...
%frustum.
%As the mesh refinement algorithm is an iterative one,
%\citeauthor{Centelles:2014} store the results of intermediate iterations
%to GPU memory.
%By reusing said results in subsequent frames, the number of iterations
%required to reach the desired mesh granularity can be reduced significantly.
%
...
%\citeauthor{article:whitecaps}
%determine the locations of the whitecaps on the water surface
%via the Jacobian determinants of the horizontal surface displacements.
%More recently, \citet{Chen:2017} have shown a computationally less expensive
%approach, where the vertical acceleration of the wave crests serves
%as a criterion for whitecap generation.
}
...
we may highlight the cornerstones of this thesis.
...
combines said lighting scheme with \citeauthor{course:simulatingocean}'s
choppy wave algorithm. Third, \citet{article:whitecaps} extend the work of
\citet{misc:oceanlightingfft} with whitecaps.
...
All three, \citet{article:oceanlighting, misc:oceanlightingfft}, and 
\cite{article:whitecaps} employ the meshing scheme by \citet{Hinsinger:2002}.
The latter severely restricts the camera's freedom of movement, therefore
we will replace it with the improved version by \citet{thesis:johanson}.
Moreover, as the work by \citet{misc:oceanlightingfft} contains
only a single wave spectrum \citep{article:Elfouhaily1997}, we will
evaluate an additional set of wave spectra with regard
to their suitability for the above lighting scheme.
Last, to improve upon the balance between model detail and computational
workload, we will implement a multi-resolution variant of the choppy wave
algorithm.
%and with the wave spectrum described in \citet{article:Elfouhaily1997}
%\citet{misc:oceanlightingfft} and \citet{article:whitecaps}
%
