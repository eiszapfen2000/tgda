...
illustrated: starting point is the model space~\subref{fig:subfigmodelspace},
next our sample model ist rotated and positioned in the world
\subref{fig:subfigworldspace}. Since we want to view the scene from within world
space we add an observing camera~\subref{fig:subfigworldspacewithcamera}. The
camera frustum is highlighted in red. At last, ~\subref{fig:subfigviewspace}
...
\begin{figure}
\centering
\subfigure[View frustum]
{
  \includegraphics[scale=1.1]{Images/Frustum.pdf}
  \label{fig:subfigviewfrustum}
}
\subfigure[Canonical Cube]
{
  \includegraphics[scale=1.0]{Images/CanonicalCube.pdf}
  \label{fig:subfigcanonicalcube}
}
\caption[View frustum, canonical cube]{Blabla ~\subref{fig:subfigviewfrustum},
and blabla \subref{fig:subfigcanonicalcube}.}
\label{fig:ViewToNDC}
\end{figure}

\FIXME{Images showing parallel and perspective projection conceptually and
actual samples.}
\FIXME{Images showing camera frustum for parallel and perspective projection}
