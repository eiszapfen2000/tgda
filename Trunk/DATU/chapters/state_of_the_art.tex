...
\begin{figure}
 ...
 \subtop[]
 {
  \includegraphics[scale=0.425]{figures/ProjectedGrid_UniformSS}
...
\cite{article:oceanlighting} augment the work of \citeauthor{Hinsinger:2002} in
...
are to be evaluated. \cite{Mitchell:2005} and \cite{Thon:2000}
...
GPU implementation of the ocean wave generation method presented in
\cite{course:simulatingocean}. To improve rendering performance,
...
The result is a detailed ocean surface with coarse underlying geometry.

\cite{misc:oceanlightingfft} combines the adaptive lighting scheme
from \cite{article:oceanlighting} with an ocean surface synthesized
from a wave spectrum. Wave heights, gradients, and
horizontal displacements for the choppy wave algorithm are computed
entirely on the GPU. In an additional step, \cite{article:whitecaps}
add whitecaps to the ocean lighting model.

As ocean surfaces generated from wave spectra allow for seamless tiling,
one may need to make sure to remove tiling artifacts i.e. the viewer
shall not notice that the ocean surface repeats itself in all directions.
The approach taken by~\cite{Rydahl:2009, NVIDIA:Ocean} is to generate
additional noise on the ocean surface. \cite{misc:oceanlightingfft,article:whitecaps}
on the other hand compute not just one ocean surface tile, but up to four,
where each one samples a different part of the wave spectrum. Even though
such an approach is unable to remove periodicity entirely, it is increased
to the least common multiple of the different tiles.
