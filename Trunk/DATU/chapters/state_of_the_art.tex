...
\cite{Kryachko:2005} superposition of handcrafted heightmaps, tiled in space and time
four heightmaps for lighting, two heightmaps (with largest scale) for displacement, 
samples heightmaps in vertex shader to compute displacement
radial grid, centered at the camera position, tesselated such that it provides
more detail close to the viewer
still most of the mesh is clipped, because out of frustum

\cite{Hinsinger:2002} quads in screenspace, projected onto undisturbed surface plane
...
\cite{thesis:johanson} projected grid, improves upon Hinsinger meshing approach
by adapting better to viewing conditions

\cite{article:oceanlighting} sum of gerstner waves
highly advanced lighting

\cite{Cui:2004}\cite{Hinsinger:2002}
\cite{Lachman:2007, Chiu:2006, Hu:2006,Mitchell:2005}
\cite{NVIDIA:WaveWorks, NVIDIA:Ocean}


\cite{Schneider:2001} vertex shader perlin noise/turbulence, forward differences for normals
\cite{Isidoro:2002} vertex shader sum of sinusoids to distort geometry and normals, dynamic combination of bumpmaps for ripples
\cite{Chen:2007} vertex shader sum of sinusoids, dynamically generated bumpmap for more surface detail
\cite{Finch:2004} vertex shader, sum of Gerstner waves, filter to small wavelengths, additional sum for detailed surface normals
