...
Instead of using sinusoids,~\cite{Fournier:1986} build on Gerstner's work
...
sea is assumed as a linear superposition of sinusoids with many different
\wavelengths, frequencies and phases, travelling in different directions. The wave
spectrum gives the distribution of wave energy among different wave frequencies
or \wavelengths. Given horizontal coordinate $\mvec{x}$, we may compute the
...
and time. Four heightmaps are summed for lighting computations, where two of them
...
\cite{Mitchell:2005} describes a
...
required for~\citet{course:simulatingocean}'s choppy wave algorithm
