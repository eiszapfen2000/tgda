...
Thus, $h$ as defined by Equation~\ref{eq:dft_h_k_t} has to be computed
whenever one requires a new keyframe for the animated ocean surface.
...
%\begin{tabular}{cccc}
	...
	%\hline
	%\multicolumn{2}{c|}{\multirow{2}{*}{Q3}} & \multicolumn{2}{c}{\multirow{2}{*}{Q4}} \\
	%\multicolumn{2}{c|}{\multirow{2}{*}{}} & \multicolumn{2}{c}{\multirow{2}{*}{}} \\
%\end{tabular}
%\end{center}
%\label{tab:multicol}
...
\end{equation*}
The \wavevector $\mvec{k} = (0,0)$ gives the location of the zero frequency,
which encodes the average value of the signal in the spatial domain. In our case
that is the mean surface elevation $\mathrm{E}[\eta] = 0$.
As one may read off the above definition of the \wavevector domain, the zero
frequency is located near the center of the \wavevector domain,
where $\alpha=0$ and $\beta=0$. If we had chosen an odd resolution $N$ instead
of an even one, the zero frequency would be located precisely at the center of the
\wavevector domain. Actual implementations of the inverse DFT, such as
\citet{misc:fftw}, expect the zero frequency as the first element i.e. at the
...
the zero frequency is highlighted in red.
\subcaptionref{fig:zfreq:center} Quadrant layout of the spectrum, the quadrant containig the zero frequency is highlighted in red.
...
\subcaptionref{fig:zfreq:upperleft} The quadrant layout we seek, where the zero frequency is held by the quadrant at the origin. Compared to
\subcaptionref{fig:zfreq:center} the quadrants have been diagonally swapped.
\subcaptionref{fig:spectrum:zfreq:upperleft} The spectrum from \subcaptionref{fig:spectrum:zfreq:center}, rearranged so that the zero frequency is located at the origin.
}
\end{figure}
%
