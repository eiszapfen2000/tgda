...
may proceed with an overview of our showcasing application which puts all the
constituent parts into a coherent whole. We implemented a compact demo
...
are not uniformly spaced, though.
}
...
independent of the actual viewing situation. The vertices projected onto the
$y=0$ plane are displaced vertically and horizontally by surface elevation
$y(\mvec{x},t)$ and displacement $\mvec{D}(\mvec{x},t)$ respectively. After
that, all vertices are projected normally on the screen via the viewing
pipeline.
%
...
\citet{article:oceanlighting} for two reasons. First, it gives visually
pleasing as well as believable results. Second, it has been adapted in such a
way that wave spectra become an integral part of the model
\citep{misc:oceanlightingfft}.
