...
we obtain surface slope vector $\mvec{s}$ as follows:
\begin{equation}
...
$h_0(\mvec{k},t)$, we may compute the surface slope vector $\nabla\eta(\mvec{x},t)$
by computing 


\begin{align}
\label{eq:dft_2d_partial_derivative_x}
 \dpd{f(\mvec{x})}{x} = \sum_{\mvec{k}}d(k_{x}, 1)~g(\mvec{k})~\mathrm{e}^{\mathrm{i}\transpose{\mvec{k}}\mvec{x}} \\
\label{eq:dft_2d_partial_derivative_y} 
 \dpd{f(\mvec{x})}{y} = \sum_{\mvec{k}}d(k_{y}, 1)~g(\mvec{k})~\mathrm{e}^{\mathrm{i}\transpose{\mvec{k}}\mvec{x}}
\end{align}
...
Given the surface slope vector $\mvec{s} = \left(\pd{\eta(\mvec{x},t)}{x}, \pd{\eta(\mvec{x},t)}{y}\right)$,
