...
\caption{Top row: reconstructed spectra of patterns with sizes reduced by
 $\varphi^{-1}$. Bottom row: reconstructed spectra of patterns with sizes
reduced by $1-\varphi^{-1}$.}
...
the same resolution $N$, but size $L$ reduced by the two different factors.
...
\end{groupplot}
\end{tikzpicture}
\caption{Assembly of the spectra from Figure~\ref{fig:sampling_pattern_size_reduction}.}
...
common in computer graphics: combine low resolution geometry augmented with
...
the same pattern sizes, it follows that all samples are equidistant. Top row
employs half the resolution of bottom row, hence it covers only half of the
wavenumber range.}
\end{figure}
%
