...
\begin{equation}
  \diff[n]{f(x)}{x} = \sum_{k}(\mathrm{i}k)^n~g(k)~\mathrm{e}^{\mathrm{i}kx}
\end{equation}
...
\label{eq:dft_derivative}
  \diff[n]{f(x)}{x} &= \sum_{k}d(k, n)~g(k)~\mathrm{e}^{\mathrm{i}kx} \\
\label{eq:dft_derivative_correction}
  d(k, n) &= \begin{cases}
                   0 &\text{if $n$ odd, $N$ even, and $k = \ceil{-\frac{N}{2}}\frac{2\pi}{L}$,} \\
                   (\mathrm{i}k)^n &\text{else.}
                   \end{cases}
\end{align}
%
We may go back to the two dimensional case, with our original domains $\mvec{x}$
and $\mvec{k}$. The resolution of said domains is $N \times N$, the size of the
spatial domain is $L \times L$. Let $g(\mvec{k})$ be the Discrete Fourier Transform
of $f(\mvec{x})$, then we may write the inverse Discrete Fourier Transform in
two dimensions as follows:
\begin{equation}
 f(\mvec{x}) = \sum_{\mvec{k}}g(\mvec{k})~\mathrm{e}^{\mathrm{i}\transpose{\mvec{k}}\mvec{x}}
\end{equation}
Similar to Equation~\ref{eq:dft_derivative}, and by reusing Equation~\ref{eq:dft_derivative_correction}
because $N$ and $L$ are equal for both dimensions, we find the first order partial
derivatives of $f(\mvec{x})$:
\begin{align}
\label{eq:dft_2d_partial_derivative_x}
 \diffp{f(\mvec{x})}{x} = \sum_{\mvec{k}}d(k_{x}, 1)~g(\mvec{k})~\mathrm{e}^{\mathrm{i}\transpose{\mvec{k}}\mvec{x}} \\
\label{eq:dft_2d_partial_derivative_y} 
 \diffp{f(\mvec{x})}{y} = \sum_{\mvec{k}}d(k_{y}, 1)~g(\mvec{k})~\mathrm{e}^{\mathrm{i}\transpose{\mvec{k}}\mvec{x}}
\end{align}

The surface slope vector consists of the first order partial derivatives
of the surface elevation. We leave it as an exercise for the reader to substitute
the terms $\eta(\mvec{x},t)$ and $h_0(\mvec{k},t)$ into
Equations~\ref{eq:dft_2d_partial_derivative_x}~\ref{eq:dft_2d_partial_derivative_y}.
Given the surface slope vector $\mvec{s} = \left(\diffp{\eta(\mvec{x},t)}{x}, \diffp{\eta(\mvec{x},t)}{y}\right)$,
where $\mvec{s}$ lies in the xz-plane, we may compute the surface normal vector as
$\mvec{n} = (-\mvecx{s}, 1, -\mvecy{s})$.

$\eta(\mvec{x},t)$ gives us surface
elevation, to find surface slope we need to compute the partial derivatives
of $\eta$ in both spatial dimensions. Based on Equation~\ref{eq:dft_derivative}
and Equation~\ref{eq:dft_derivative_correction}, we may write:
