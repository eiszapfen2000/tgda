...
brighter means larger values, darker means lower values. Because both spectra
that represent displacement vectors $\mvec{D}$ are hermitian, we are again able 
to apply Equation~\ref{eq:idft_combined} and obtain both components with
just one inverse Fourier Transform.\\


Given the displacement vectors $\mvec{D}$, as well as the original grid
points $\mvec{x}$, according to Tessendorf we may compute the displaced
gridpoints $\mvec{y}$ as follows:
\begin{equation}
...
is correct if $\lambda \leq 0$, otherwise the effect is the opposite of what
we want to achieve. Looking at Equation~\ref{eq:displacement_gridpoints},
...
the waves' peaks are sharpened and the waves' valleys are broadened, see
Figure~\ref{fig:grid_displaced}.
%
\begin{figure}
\centering
\begin{tikzpicture}
...
\addplot[
	...
	{figures/u_30_500km_x_dx_h.dat};
\addlegendentry{$x - \lambda D_x$}
\coordinate (spypoint1) at (axis cs:47.9,0.73);
\coordinate (magnifyglass1) at (axis cs:47.9,2);
\coordinate (spypoint2) at (axis cs:-44,0.45);
\coordinate (magnifyglass2) at (axis cs:-44,2);

%\draw [blue] (45,0.4) rectangle (48,1);
\end{axis}
