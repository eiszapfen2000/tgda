...
We employ one resolution for all patterns, therefore our focus is on size to
reduce wavenumber overlap. We need at least one large pattern to avoid
repetitive artifacts as much as possible, and a small one for closeup detail.
In addition, we have to look out for the different sizes not to be integral
multiples of each other, otherwise the surface may seem repetitive at a smaller
scale than expected. We chose to tackle both requirements by employing multiples
based on the golden ratio $\varphi = (1 + \sqrt{5})/2$. The user sets the
largest desired pattern size, each following size is reduced either by
the factor $\varphi^{-1}$ or by the factor $1 - \varphi^{-1}$.
The former generates more overlap in the wavenumber range than the latter, but
is better at reducing repetitive artifacts.

Care has to be taken not to sample the same
...
Note: The typical profile of a one dimensional wavenumber spectrum suggests
...
the high wavenumber range, see Fr\'{e}chot~\cite{article:frechot2007}. Because
