...
\begin{itemize}
 ...
 $\mvec{k}$, but the same underlying size $L$.
 \item Extract $\Theta(\mvec{l})$ from $\Theta(\mvec{k})$.
 \item For each $\Theta(\mvec{k})$ extracted, also copy its associated random
 number pair $(\xi_r,\xi_i)$.
\end{itemize}
...
the amplitude associated with a wavevector. For us to be able to reproduce the
same random surface at different resolutions, the pairing between amplitude and
wavevector needs to be the same across resolutions.
