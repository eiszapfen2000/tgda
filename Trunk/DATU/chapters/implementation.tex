...
are required to include as many different wavenumbers as possible into our sum
of waves. If we generate one pattern only, based on size and resolution, we
either sparsely sample inside a major range of wavenumbers, or we densely sample
a small range of wavenumbers. The former results in a poor reconstruction of a
...
the high wavenumbers, see NEED CITATION. Because we employ a discrete Fourier
...
$\mvec{k}$: the spacing between subsequent coordinates is constant. Hence, we
are not allowed to employ a sampling scheme where $\Delta k$ changes dependent on
the current position on the wavenumber profile.
