...
$N$. The minimal wavenumber $\abs{k_{min}} > 0$ representable by the wavevector
domain is $k_{min} = 2\pi/L$, it depends only on size. Moreover, the distance
between subsequent sample points in both dimensions, $\Delta k$, is defined as
$\Delta k = k_{min} = 2\pi/L$. Hence, $\Delta k$ depends only on size. Therefore,
by setting size $L$, we do not only define the minimal wavenumber we are able to sample, but also define how densely we are able to sample the spectrum.
