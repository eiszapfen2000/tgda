...
The ocean lighting scheme by Bruneton et al.
\cite{article:oceanlighting}\cite{misc:oceanlightingfft} does not account
for the breaking of waves on the open ocean. Neither the geometric deformations,
nor the changes in reflectance caused by foam and spray, so called whitecaps,
are incorporated in their model. But an extension of their FFT based work
\cite{misc:oceanlightingfft}, namely Dupuy et al.\cite{article:whitecaps},
includes whitecaps. The latter is based on a GPU optimized version of Tessendorf's
approach\cite{course:simulatingocean} to locate self intersections on the ocean
surface via the Jacobian determinant of the horizontal displacements, see the
discussion in Section~\ref{sec:self_intersections}.
Additional preprocessing step which computes the two terms, $k_i$ and $k_i^2$
from Equation 7 and 8 in \cite{article:whitecaps}.


% and related
%Equations \ref{eq:jacobian_of_displacement}\ref{eq:jacobian_of_displacement_multi}.



which employs the Jacobian $\mvec{J}(\mvec{x},t)$
of horizontal displacements $\mvec{D}(\mvec{x},t)$ to locate self intersections
