...
spectra into the demo application, making sure the results were the same as in
the MATLAB version. In addition, we chose to employ the same library for
Fast Fourier Transforms as MATLAB, namely FFTW\cite{misc:fftw}.

We use one thread to generate frequency spectra, a second thread to apply the inverse
DFT on said spectra, while the main thread does all the rendering.

Projective Grid
Preetham Sky + Sun Disc
Bruneton Seamless Ocean Lighting
Ocean Whitecaps
Reinhard Tonemap \cite{Reinhard:2002}
