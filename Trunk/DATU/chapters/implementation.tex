...
Equation 4 in\cite{Reinhard:2002}, because it is straightforward to implement and
...
\end{equation}
where $N$ is the number of pixels in the image, $L_w(x,y)$ is the luminance for
pixel $(x,y)$, and $\delta$ is small positive value to avoid the singularity at
$\log(0)$. For $L_w(x,y)$, we first convert the pixel's RGB color to CIE XYZ, and
then use the luminance component Y. We write $\log(\delta + L_w(x,y))$ values to a separate
render target for which we generate mipmaps. The highest mipmap level, which consists
of one pixel, gives us $\bar L_w$.
