...
%
\subsection{Tiling}
We have already mentioned that we reduce tiling artifacts as much as possible
by superimposing a set of patterns of different size \citep{misc:oceanlightingfft,
article:whitecaps}. To be able to do so we require at least two patterns, a
large one which defines the general shape of the ocean surface, and a small one
for closeup detail.
...
size by factor $f$, where $f \in \{\varphi^{-1}, 1 - \varphi^{-1}\}$.
Given initial pattern size $L$, we may compute size $L_i$ of the successive
patterns as follows:
\begin{align}
 L_i = f^{i} L && 0 < i < l
\end{align}
