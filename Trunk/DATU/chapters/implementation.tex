...
Given the displacement vector $\mvec{D}$, as well as the original grid
point $\mvec{x}$, according to Tessendorf we may compute the displaced
gridpoint $\mvec{y}$ as follows:
\begin{equation}
\label{eq:displacement_gridpoints}
 \mvec{y}(\mvec{x},t) = \mvec{x} + \lambda \mvec{D}(\mvec{x},t)
\end{equation}
...
\addplot[
	...
	{figures/u_30_500km_x_dx_h.dat};
\addlegendentry{$[x + \lambda D_x, \eta(x)]$}
\end{axis}
\end{tikzpicture}
...
\end{figure}
%
See Figure~\ref{fig:grid_displaced} for a comparison between an example instance 
of a wave profile and its displaced version.
%
