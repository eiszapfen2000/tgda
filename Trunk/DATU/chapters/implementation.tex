...
\label{eq:displacement_gridpoints}
 ...
 \end{bmatrix}
\end{equation}
where $u$ is a user-controlled parameter to scale the importance of the
...
not.
%
\subsubsection{Slopes and Displacements}
%
...
$\mvec{x} + u\mvec{D}(\mvec{x},t)$, then we obtain the two dimensional surface 
slope $\mvec{s}$ as follows:
\begin{equation}
...
%
\subsubsection{Self-Intersections}
%
\begin{figure}
\centering
\begin{tikzpicture}[
...
the Jacobian matrix of function $\mvec{y}$, then we may write:
%
\begin{equation}
 \mvec{J}(\mvec{x},t) =
 \begin{bmatrix}
...
Transform, the resulting data may be seamlessy tiled on the ocean surface. 
...
pattern, but a set, where each pattern samples a different part of the spectrum. 
Although we are not able to remove periodicity from the wave field, this way we 
increase it to the least common multiple of the pattern periods.

At this point, one perturbation pattern consists of surface elevation $\eta$, 
displacement vector $\mvec{D}$, and slope vector $\mvec{s}$. We know that 
surface elevation $\eta$ and displacement vector $\mvec{D}$ are periodic, 
therefore their first order partial derivatives are periodic, too. As such, it 
is straightforward to extend our computations of displaced vertex coordinates 
as well as slopes to incorporate not just one perturbation pattern, but a set 
of patterns.
 Let $l \in 
\mathrm{N}^+$ represent the number of % In short, $\mvec{x}_i,\mvec{k}_i$
