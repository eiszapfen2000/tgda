...
\caption{\subcaptionref{sfig:derivative_heights} Example instance of a 
heightfield (Unified spectrum, $U_{10}=30m\cdot s^{-1}$, $F=500km$). 
\subcaptionref{sfig:derivative_x} The heightfield's first order partial 
derivative in the X-direction. \subcaptionref{sfig:derivative_z} The 
heightfield's first order partial derivative in the Y-direction.
}
...
\end{equation}
As we need to find the surface slope vector, we need to compute the surface
elevation's first order partial derivatives. Given surface elevation $\eta(\mvec{x},t)$,
we obtain the two dimensional surface slope vector $\mvec{s}$ as follows:
\begin{equation}
\label{eq:dft_slope_2d}
 \mvec{s} = \left[\dpd{\eta(\mvec{x},t)}{x}, \dpd{\eta(\mvec{x},t)}{y}\right]
\end{equation}
...
Figure~\ref{fig:displacements} depicts an example instance of surface 
elevation $\eta$ and its associated slope vector.\\
...
\begin{equation}
 \mvec{n} = \frac{(-\mvecx{s}, 1, -\mvecy{s})}{\norm{(-\mvecx{s}, 1, -\mvecy{s})}}
\end{equation}
%
...
\caption{To the left, an example instance of a heightfield (\emph{Unified} 
spectrum, $U_{10}=30m\cdot s^{-1}$, $F=500km$), next to it its first order 
partial derivatives.}
...
%\label{eq:dft_displacement_correction}
 d(l, k) &= \begin{cases}
             0 &\text{if $N$ even, and $k = 0$ or $k = \ceil{-\frac{N}{2}}\frac{2\pi}{L}$,} \\
             -\mathrm{i}\frac{l}{k} &\text{else.}
            \end{cases}
\end{align}
where the term $d(l, k)$ makes sure the resulting spectrum is hermitian.
Given the displacement vectors $\mvec{D}$, as well as the original grid points $\mvec{x}$, we may compute
the displaced gridpoints $\mvec{y}$ as follows:
\begin{equation}
 \mvec{y} = \mvec{x} - \lambda \mvec{D}(\mvec{x}, t)
\end{equation}
where $\lambda$ is a user-controlled parameter to scale the importance of the displacement vector.
As one can see, the wave heights are not altered, instead the grid points on the XZ-plane
are transformed based on the spatial structure of the height field. This particular transformation
accomplishes the effects we sought, the waves' peaks are sharpened and the 
waves' valleys are broadened.
