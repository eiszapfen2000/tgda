...
to surface height $\eta$, displacement vector $\mvec{D}$, and the first order
partial derivatives of $\eta$, we also need to compute a partial derivative of
...
In practice, four different perturbation patterns suffice to obtain satisfactory
results \citep{article:whitecaps}.
...
resolution than the other datasets. We conclude, that the potential reduction of
computational workload is solely dependent on the actual difference between
the two resolutions.
...
periods to be able to sample as much of the spectrum as possible, and last we
discuss our multi-resolution approach.
