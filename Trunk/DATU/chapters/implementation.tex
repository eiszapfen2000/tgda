...
on wave spectra. Now we may move the focus to computer graphics algorithms in
conjunction with wave spectra. As described by \citet{course:simulatingocean},
we may capitalize on the spectral representation of ocean waves on three fronts.
First, we may obtain high quality normal vectors by deriving the ocean surface's
slope vectors in frequency space. Second, the choppy wave algorithm may add
displacements to the ocean surface for it to better resemble an agitated sea
where the waves have deep throughs and sharp crests. Third, based on the 
first-order partial derivatives of aforementioned displacements one may be able
to derive the locations on the ocean surface where whitecaps may arise. All three
points lead to additional spectra, all derived directly from the ocean surface
elevation spectrum, burdening us with additional Fourier Transforms.
