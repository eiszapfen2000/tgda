...
The same applies to the random ocean surface spectrum $h_0$ as defined by
Equation~\ref{eq:dft_h0_k}, and to $h$ as defined by Equation~\ref{eq:dft_h_k_t},
which animates said surface spectrum.
If we compute $h_0(\mvec{h})$ as well as $h(\mvec{h},t)$ and store the results,
then we may generate a low-resolution variant of the surface simply by
extracting the respective subsets $h_0(\mvec{l})$ and $h(\mvec{l},t)$.
%
%
\begin{figure}[p]
\centering
