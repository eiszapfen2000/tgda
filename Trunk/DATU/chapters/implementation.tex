...
%As one may already guess, the improvement in computational load is dependent on
%the reduction in resolution. Moreover, 
%
...
It is written in Objective-C and employs the \citet{misc:gnustep}
Base Library which provides container classes as well as basic algorithms.
We use the compatibility profile of OpenGL 3.3 \citep{misc:opengl,misc:opengl33}
for rendering, where the GLFW library~\citep{misc:glfw}
...
energy spectra into the demo application, making sure the results match the ones
of the MATLAB version. In addition, we chose to employ the same library for
...
derivatives, the spectrum type, wind velocity $U_{10}$, the size $L$ of the
largest pattern, and fetch $F$.
...
grid~\citep{Hinsinger:2002,thesis:johanson}, which is based on a simple
...
displaced vertically and horizontally by surface elevation $y(\mvec{x},t)$
and displacement $\mvec{D}(\mvec{x},t)$ respectively. After that, all vertices
