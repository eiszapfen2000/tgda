...
would like to sample the spectrum at as much wavenumbers as possible, we need to
...
region of overlap. As for the different sizes of the spatial domains, we need
them not to be integral multiples of each other, otherwise the surface may seem
repetitive at a smaller scale than expected. Instead, we employ multiples based
on the golden section $\varphi = \frac{1 + \sqrt{5}}{2}$: start with the largest
desired pattern size, reduce each following size by a factor $\varphi^{-1}$. If
this does not give satisfying results, the user may always choose all sizes by
hand.
