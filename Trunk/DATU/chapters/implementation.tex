...
\subcaptionref{fig:zfreq:upperleft} The quadrant layout we seek, where the zero frequency component is held by the quadrant at the origin. Compared to \subcaptionref{fig:zfreq:center} diagonally opposite quadrants have been swapped.
\subcaptionref{fig:spectrum:zfreq:upperleft} The spectrum is rearranged according to the quadrant layout from \subcaptionref{fig:zfreq:upperleft}, placing the zero frequency component at the origin.
}
...
where $\alpha=0$ and $\beta=0$. Actual implementations of the inverse DFT, such as
\citet{FFTW05}, expect the zero frequency component as the first element i.e. at the
...
%
\begin{align*}
 h_0(\mvec{k}(\alpha\Delta k,\beta\Delta k)) = h_0(\mvec{k}((\alpha + lN)\Delta k, (\beta +mN)\Delta k) && l,m \in \mathbb{Z} 
\end{align*}
%
...
\subcaptionref{fig:symmetry:odd:example} Example spectrum matching the hermitian matrix layout from \subcaptionref{fig:symmetry:odd:layout}.
The zero frequency component is highlighted in red.
\subcaptionref{fig:symmetry:even:layout} Hermitian matrix ($N=5$).
\subcaptionref{fig:symmetry:even:example} An even-sized spectrum ($N=4$), which repeats itself in both directions,
such as for all elements of the spectrum there is a pair which satisfies $\mathcal{F}(-\mvec{k})=\mathcal{F}(\mvec{k})^*$,
matching the hermitian matrix layout from \subcaptionref{fig:symmetry:even:layout}.
