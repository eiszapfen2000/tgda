...
patterns, where each pattern is different in size and therefore samples a different
part of the spectrum.
The superposition of patterns of different size is not able to entirely remove
periodicity from the wave field, but still, periodicity is reduced to the least
common multiple of the pattern periods.
Moreover, as each pattern samples a different part of the spectrum, we are able
to extend the sum of waves to include shorter wavelengths, even as far as into
the gravity-capillary wave domain. Thus, given a sufficient number of patterns,
...
periods to be able to reduce tiling artifacts as well as sample as much of the
spectrum as possible, and last we discuss our multi-resolution approach.
%
...
reconstructions of one specific one-dimensional \wavenumber spectrum, where one
can see the effects of size $L$ and resolution $N$ on the \wavenumber set $k_d$,
and thus on the reconstructed pattern.
%
...
Because our implementation shares the same dataset resolution(s) across all
patterns, we are required to exclusively focus on pattern size to reduce
\wavenumber overlap.
...
\citet{article:frechot2007}. Because we employ a \DiscreteFourierTransform to
...
Recall Equation~\ref{eq:dft_h0_k}, where we synthesize a random
