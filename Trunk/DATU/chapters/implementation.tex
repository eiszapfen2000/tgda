...
Two, displacements to transform the gentle sinusoidal form of the sea into a more agitated one, similar to Gerstner waves.
...
whitecaps may arise.
%Given a wave spectrum, one is able to synthesize a heightfield taking the form of an
%animated ocean surface. Still surface elevation alone 
%The choice of wave spectrum model defines what kind of
%sea one may simulate, e.g. whether it is a fully developed sea or a developing sea,
%or whether it includes capillary waves or not.
%
%
%The previous chapter discussed the steps necessary to synthesize ocean surface
%heightfields based on wave spectra, where the latter represent an oceanographic
%framework to describe the distribution of wave energy among ocean surface waves
%dependent on parameters such as wind speed and distance from shore.
%From now on we may focus on how to best embed wave spectra into computer graphics
%algorithms.
%Now we may focus on integrating wave spectra into computer graphics algorithms.
%The previous chapter discussed the conceptual background of wave spectra,
%as well as the steps necessary to synthesize ocean surface heightfields based
%on such. From now on we may focus on computer graphics algorithms in conjunction
%with wave spectra.
%
%As described by \citet{course:simulatingocean},
%we are able to capitalize on the spectral representation of ocean waves on three fronts.
%First, for lighting purposes we may obtain high quality normal vectors by
%deriving the ocean surface's slope vectors in frequency space.
%Second, we may add displacements to the ocean surface for it to better resemble
%an agitated sea, where the waves have deep troughs and sharp crests.
%Third, based on the first-order partial derivatives of aforementioned
%displacements one may be able to deduce the locations on the ocean surface where
%whitecaps may arise.
Each of the three points requires us to construct additional spectra,
...
%Section~\ref{sec:discrete_fourier_transform} describes opportunities to optimise
...
% \begin{itemize}
% \item first, reduce the amount of data to synthesize per frame to a minimum
% \item adapt data so that it fits the best case for the FFT algorithm
% \item second, FOurier Transform at half the cost
% \item third, two Fourier Transforms at the cost of one
% \end{itemize}
...
%
\subsection{Combined Inverse Fourier Transform}
...
lead of the continuous Fourier Transform~\citep{book:bracewell2000fourier},
then we may compute the $n$th derivative of $f(x)$ as follows:
\begin{equation}
...
first component of the displacement vector. \subcaptionref{sfig:displacement_z} The 
second component of the displacement vector.}
...
\end{groupplot}
\end{tikzpicture}
\caption{FIXME: Reconstructions of one and the same spectrum. Top and bottom row use
