...
describes a \FourierTransform based method to produce such choppy waves. The concept is simple: displace
...
ocean waves. Because the sampling process is done by means of a \FourierTransform,
...
To be able to do so we require a minumum of two patterns: a large one which defines
...
\emph{Note}: If one deems it necessary, he or she may always choose pattern
sizes via a different mechanism, or based on other irrational numbers, or even by
hand as \citeauthor{article:whitecaps} did.
Still, we have found Equation~\ref{eq:pattern_size_i} to be a surprisingly simple
as well as adequate solution to the issue at hand.
...
If we generate only one pattern, based on size and resolution, we either
sparsely sample inside a large \wavenumber range, or we densely sample inside a
small \wavenumber range.
If, on the other hand, we generate multiple patterns of differing size, then
the patterns' \wavenumbers ranges complement each other, see
Figure~\ref{fig:sampling_pattern_size_reduction}. Thus, we are able
...
based on pattern size. Given the enlarged \wavenumber range, we may include
shorter \wavelengths into the sum of waves, allowing us to reproduce sufficient
detail for close-ups of the water surface.
...
reconstructions of the wave energy spectrum, based on the two different
dataset resolutions. For each set of reconstructions we still need to assemble
the overlapping spectral data into one coherent spectrum. As before, the
transition from one
reconstruction to the next takes place at the maximum \wavenumber of the
reconstruction with the smaller \wavenumber range. In addition, as shown in
Figure \ref{fig:sampling_different_resolutions_pattern_assembly}, we must take
into consideration that the two sets of spectral data have different \wavenumber
ranges, therefore the transitions may take place at different \wavenumbers.
%
...
\FastFourierTransforms as MATLAB, namely FFTW\cite{misc:fftw}. FFTW does
