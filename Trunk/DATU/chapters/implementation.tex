...
is an even function and the imaginary part is odd. A function $f(n)$ is said to be~\emph{even} if
$f(-n) = f(n)$. On the other hand, a function $f(n)$ is said to be~\emph{odd} if $f(-n) = -f(n)$,
with $f(0) = 0$.
