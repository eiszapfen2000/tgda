...
displacement vector $\mvec{D}$, and the slope of $\eta$. Because we compute the
surface displacements as a sum of waves, it is straightforward to extend our
...
\begin{equation}
 \left[\dpd{z(\mvec{x},t)}{x},\dpd{z(\mvec{x},t)}{y}\right] = \sum\limits_{i=0}^{l-1}\left[\dpd{\eta_i(\mvec{x},t)}{x}, \dpd{\eta_i(\mvec{x},t)}{y}\right]
\end{equation}
Furthermore, we may write the grid point displacement, which incorporates
a set of patterns, as follows:
\begin{equation}
...
\begin{equation}
 \mvec{J}(\mvec{x},t) =
 \begin{bmatrix}
   1 + \sum_i u_i\dpd{D_{xi}(\mvec{x},t)}{x} & \sum_i u_i\dpd{D_{xi}(\mvec{x},t)}{y} \\
   \sum_i u_i\dpd{D_{yi}(\mvec{x},t)}{x} & 1 + \sum_i u_i\dpd{D_{yi}(\mvec{x},t)}{y}
 \end{bmatrix}
\end{equation}
