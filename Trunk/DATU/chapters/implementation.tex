...
\subcaptionref{fig:symmetry:even:example} An even-sized spectrum ($N=4$).
If we replicate the spectrum until both its dimensions have an odd-numbered size ($N=5$),
then one can see that for all elements of the spectrum there is a pair which satisfies the hermitian
...
Equation~\ref{eq:dft_h_k_t} as well as the hermitian given by Equation~\ref{eq:dft_h_k_t_hermitian}.
