...
Projected Grid\cite{Hinsinger:2002}\cite{thesis:johanson}
...
The overall tonemapping algorithm is simple, first convert each source pixel
from RGB to CIE xyY, where the luminance component Y acts as the pixel's
''world'' luminance $L_w$. Next, we update the luminance component Y according to
Equation~\ref{eq:reinhard_operator}, and as a last step convert the luminance
adjusted xyY color back to RGB and store it in the target. The actual colorimetric
conversion functions we use are documented in \cite{misc:colorconversion}.
The log-average luminance $\bar L_w$ is precomputed for each frame by simple means
