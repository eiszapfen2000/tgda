...
Let $g(k)$ be the discrete Fourier Transform of $f(x)$ and let $N$ be the number
of elements of $g(k)$, then we may write the discrete inverse Fourier Transform as follows:
\begin{equation}
 f(x) = \sum_{k}g(k)~\mathrm{e}^{\mathrm{i}kt}
\end{equation}
Note, that we omit the scaling factors involved in the Fourier transform
because in the context of this work they are not needed. Given the discrete
inverse Fourier Transform, we may compute the derivative of $f(x)$ as follows:
\begin{equation}
  \diff{f(x)}{x} = \sum_{k}\mathrm{i}k~g(k)~\mathrm{e}^{\mathrm{i}kt}
\end{equation}
If $N$ is even, then we need to modify the above equation. Let $\alpha \in \mathbb{N}$
with $-\frac{N}{2} \leq \alpha < \frac{N}{2}$, let $x \in \alpha \frac{L}{N}$,
and let $k \in \alpha \frac{2\pi}{L}$, then:
\begin{align}
  \diff{f(x)}{x} &= \sum_{k}b(\mathrm{i}k)~g(k)~\mathrm{e}^{\mathrm{i}kt} \\
  b(\mathrm{i}k) &= \begin{cases}
                   0 &\text{if $\alpha = -\frac{N}{2}$,} \\
                   \mathrm{i}k &\text{else.}
                   \end{cases}
\end{align}
