...
\label{sec_linear_theory_ocean_waves}

Assume we observe an idealized ocean in which exactly one sine wave is travelling constantly.
The parameters defining the sine wave - amplitude, frequency, length and direction - are all fixed.
Furthermore we restrict our observation to the vertical displacement $\eta$ at location $\mvec{x}$.
To simplify the mathematics we assume a two-dimensional ocean, with the abscissa representing the
wave's direction of travel and the ordinate its vertical displacement. With these assumptions it
follows in terms of the one-dimensional parameters that:
%
\begin{align}
 \eta = Asin(kx - \omega t)\\
\intertext{with}
...
\end{align}
%
where $k$ is the~\emph{wave number},~$\lambda$ is the~\emph{wave length},~$\omega$ is the~\emph{wave frequency}
in radians per second and $f$ is the~\emph{wave frequency} in Hertz (Hz).
