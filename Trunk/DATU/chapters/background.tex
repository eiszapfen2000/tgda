...
To simplify the mathematics we assume a two-dimensional ocean, with one dimension representing the
wave's direction of travel and the other dimension the wave's vertical displacement.
...
\end{align}
%
where $A$ is the amplitude, $k$ is the~\emph{wave number},~$\lambda$ is the~\emph{wave length},
~$\omega$ is the~\emph{wave frequency} in radians per second,~$f$ is the~\emph{wave frequency}
in Hertz (Hz) and~$T$ is the wave period.
