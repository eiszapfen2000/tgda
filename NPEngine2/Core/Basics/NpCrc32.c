...
const uint32_t crc32_2bit_table[] =
{
    0x00000000L, 0x76dc4190L, 0xedb88320L, 0x9b64c2b0L
};

const uint32_t crc32_4bit_table[] =
{
...
const uint32_t crc32_8bit_table[] =
{
...
void crc32_verify_tables()
{
    ...
    {
	    uint32_t crc = pattern;

	    crc32_update_with_1bit(&crc, 0);
	    crc32_update_with_1bit(&crc, 0);

	    if (crc32_2bit_table[pattern] != crc)
	        printf("crc32_2bit_table[0x%udx] == 0x%udx (should be 0x%udx)",
		            pattern, crc32_2bit_table[pattern], crc);
    }
    ...
    for (pattern = 0; pattern < 16; pattern++)
    {
	    ...
	    if (crc32_4bit_table[pattern] != crc)
	        printf("crc32_4bit_table[0x%udx] == 0x%udx (should be 0x%udx)",
		            pattern, crc32_4bit_table[pattern], crc);
    }
...
{
    crc32_update_with_32bits(crc, (uint32_t) data);
...
#endif
#ifdef NP_64BIT_LONG
    crc32_update_with_64bits(crc, (uint64_t) pointer);
#endif
}
...
{
    const uint8_t * byte_ptr = (const uint8_t *)string;
    char ch;
...
void crc32_write_to_data( uint32_t crc, void * pointer )
{
    uint8_t * byte_ptr = (uint8_t *)pointer;
    *byte_ptr++ = (uint8_t)crc; crc >>= 8;
    *byte_ptr++ = (uint8_t)crc; crc >>= 8;
    *byte_ptr++ = (uint8_t)crc; crc >>= 8;
    *byte_ptr   = (uint8_t)crc;
}
...
#ifdef NP_64BIT_LONG
    return crc32_of_uint64((uint64_t)pointer);
#endif
}
